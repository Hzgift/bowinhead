document.addEventListener('DOMContentLoaded', () => {
  ...
  // ============================================================
  // 4. ЧАСТИЦЫ-СОЗВЕЗДИЯ (усиленные)
  // ============================================================
  const canvas = document.querySelector('.particles');
  const ctx = canvas.getContext('2d');
  let W, H, particles = [];
  const COLORS = ['#FFD700','#FFB400','#FF8C00','#FFF6D5','#FFFFFF','#fff8b0'];
  const COUNT = 160; // было 120
  ...
  class P {
    constructor() {
      ...
      this.z = Math.random();
      this.size = this.z * 2.6 + 0.5;
      this.speedX = (Math.random() - 0.5) * 0.28 * (0.3 + this.z);
      this.speedY = (Math.random() - 0.5) * 0.28 * (0.3 + this.z);
      this.color = COLORS[(Math.random() * COLORS.length) | 0];
      this.opacity = 0.35 + this.z * 0.6;
      this.twinkle = Math.random() * Math.PI * 2;
      this.twinkleSpeed = 0.015 + Math.random() * 0.03;
      // "Узлы" созвездий - крупные яркие звёзды
      this.isNode = Math.random() < 0.12;
      if (this.isNode) {
        this.size = 2.8 + Math.random() * 1.5;
        this.opacity = 0.9;
      }
    }
    update() {
    ...
    draw() {
      ...
      ctx.shadowBlur = this.isNode ? 24 : 12 * this.z;
    ...
    for (let a = 0; a < particles.length; a++) {
      for (let b = a + 1; b < particles.length; b++) {
        ...
        if (d < MAX_D) {
          ...
          // Узлы дают более яркие линии
          const mult = (particles[a].isNode || particles[b].isNode) ? 0.55 : 0.28;
          ...
          ctx.lineWidth = (particles[a].isNode || particles[b].isNode) ? 0.9 : 0.5;
          ctx.beginPath();
  ...
  animStyle.textContent = `
    ...
    @keyframes chainSpark{0%{transform:translate(-50%,-50%) scale(1);opacity:1}100%{transform:translate(calc(-50% + var(--tx)),calc(-50% + var(--ty))) scale(0);opacity:0}}
  `;
  ...
  heroBtns.forEach(btn => {
    ...
    btn.addEventListener('click', e => {
      ...
      circle.style.cssText = `position:fixed;left:${cx}px;top:${cy}px;border-radius:50%;pointer-events:none;z-index:9995;background:radial-gradient(circle,#FFD700,#FF8C00 60%,#FF5E00);transform:translate(-50%,-50%);animation:pageFill .65s cubic-bezier(.65,.05,.36,1) forwards;box-shadow:0 0 100px rgba(255,215,0,.6);`;
  ...
  window.unlockAdult = function(e, el) {
    if (!el.classList.contains('unlocked')) {
      ...
      // Красные искры (как будто цепи разорвались)
      burstSparks(cx, cy, 40, '#ff2e2e');

      // Золотые искры от замка
      ...
      // Активируем разблокировку (цепи улетят через CSS transition)
      el.classList.add('unlocked');

      // Обновляем hint
      const hint = el.querySelector('.adult-hint');
      if (hint) {
        hint.textContent = 'РАЗБЛОКИРОВАНО · ОТКРОЕТСЯ ПРИ ПОВТОРНОМ КЛИКЕ';
        hint.style.color = 'var(--gold)';
      }
    }
    // если уже unlocked — переход по ссылке сработает автоматически
  };

  const adultShake = document.createElement('style');
  adultShake.textContent = `@keyframes adultShake{0%,100%{transform:translate(0)}15%{transform:translate(-8px,2px) rotate(-1deg)}30%{transform:translate(7px,-2px) rotate(1deg)}45%{transform:translate(-5px,3px)}60%{transform:translate(5px,-3px) rotate(-.5deg)}75%{transform:translate(-3px,1px)}}`;
  document.head.appendChild(adultShake);
