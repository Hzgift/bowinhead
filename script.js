document.addEventListener('DOMContentLoaded', () => {
    ...
    window.openMedia = function(src, type) {
        ...
        if (type === 'image') {
            innerHTML = `<img src="${src}" alt="Zoomed" class="lightbox-media"><span class="lightbox-close" onclick="closeMedia()">&times;</span>`;
        } else if (type === 'video') {
            innerHTML = `<div class="video-wrapper"><iframe src="${src}" frameborder="0" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen></iframe></div><span class="lightbox-close" onclick="closeMedia()">&times;</span>`;
        }
    ...
    window.closeMedia = function() {
        ...
        setTimeout(() => {
            document.querySelector('.lightbox-content').innerHTML = ''; // Очистка для остановки видео
        }, 400);
    }
    ...
    if (cursorDot && cursorOutline) {
        ...
        const hoverElements = document.querySelectorAll('a, button, .nav-link, .filter-btn, .social-card');
    ...
    function fallbackCopy(text) {
        ...
        try { document.execCommand('copy'); showToast(); } catch (err) { console.error('Ошибка', err); }
        textArea.remove();
    }
