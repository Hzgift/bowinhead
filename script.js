document.addEventListener('DOMContentLoaded', () => {
  ...
  function burstSparks(x, y, count = 18) {
    for (let i = 0; i < count; i++) {
      ...
      s.style.cssText = `position:fixed;left:${x}px;top:${y}px;width:${2 + Math.random() * 4}px;height:${2 + Math.random() * 4}px;background:#FFD700;border-radius:50%;pointer-events:none;z-index:9998;box-shadow:0 0 14px #FFB400,0 0 6px #fff;--tx:${Math.cos(ang) * dist}px;--ty:${Math.sin(ang) * dist}px;animation:sparkFly .9s cubic-bezier(.19,1,.22,1) forwards;`;
  ...
  // Общий клик по странице (искры + волны)
  document.addEventListener('click', e => {
    if (e.target.closest('.hero-btn')) return; // герои сами обрабатывают
  ...
  heroBtns.forEach(btn => {
    ...
    btn.addEventListener('click', e => {
      ...
      // Скролл в момент, когда экран закрыт
      setTimeout(() => {
  ...
  // Специально для social-cards — спотлайт
  ...
  window.unlockAdult = function(e, el) {
    if (!el.classList.contains('unlocked')) {
      ...
      // небольшая встряска
      el.style.animation = 'adultShake .5s ease';
      setTimeout(() => el.style.animation = '', 500);
    }
    // если уже unlocked — клик пройдёт по ссылке
  };

  const adultShake = document.createElement('style');
  adultShake.textContent = `@keyframes adultShake{0%,100%{transform:translate(0)}25%{transform:translate(-6px,2px)}50%{transform:translate(5px,-2px)}75%{transform:translate(-3px,-3px)}}`;
